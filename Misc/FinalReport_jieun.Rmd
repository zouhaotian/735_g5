...
Finally, for maximization $\gamma, \alpha$, we fit a Cox proportional hazards model, using the augmented survival dataset, given $\log(h_0)^{(t+1)}$, $m_i(T_i)^{(t+1), m}$, where $m_i(T_i)^{(t+1), m} = \beta_0^{(t+1)} + \beta_1^{(t+1)}T_i + \boldsymbol{x_i'^{(T_i)}} \boldsymbol{\beta^{(t+1)}} + u_i^{m}$, and $u_i^{m}$ denotes the $m^{th}$ random effect for subject $i$.
...
First, we simulated the longitudinal responses based on the estimated values from the original `aids` data using the following formula: $Y_{ij} = 20 + (-1)*t_{ij} + (-4)*x_i + u_i + \epsilon_{ij}$, where $u_i \sim N(0, 4), \epsilon_{ij} \sim N(0, 1), x_i \sim \text{binom}(1, 0.4)$, for subject $i$ at time point $j$ where $i = 1,\dots,N$ and $j = 0,2,6,12,18$. 
...
+ `jm_filter`: Function that generates the longitudinal and survival data we need to run our three methods. 

+ `MCEM`, `fit_stan`, `RandomSurvivalForest`: Main functions that applies to our three methods MCEM, Bayesian, and random survival forest, respectively.

+ `create_cv`: Function that generates cross-validation data from our original data. Used when generating AUC and Brier Score.

+ `cal_AUC_BS`, `calc_AUC_BS_RSF`: Functions that generate AUC and BS calculation for MCEM, Bayesian and RSF.

+ `SimulateDataset`: Function that generates simulation data

+ `sim.MCEM`, `sim.Bayesian`: Functions that generate simulated datasets to check if functions for each method runs correctly.
...
```{r}
kable(mcem.result, caption = "Model Fitting Results from MCEM")
```
...
```{r}
kable(fit.result, caption = "Model Fitting Results from Bayesian")
```
