...
It is time-consuming to run MCEM and each iteration takes more than 1 minute. Although the algorithm is hard to converge and we only run 10 iterations, the estimates is still similar with the estimates given by Bayesian method below.
...
For Bayesian method, it is easy to implement in Stan. Since we got the posterior distributions of the parameters, we can easily obtain the estimates and corresponding 95% credible intervals.
