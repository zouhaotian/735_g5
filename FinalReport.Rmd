...
Introduction
	...
	Question(s) to be explored discussed and learn
	Brief description of data/characteristics, how it related to addressing question(s)
...
Methods
	...
	Discussion of Module 3 machine learning used
...
R package
	Should pass package tests
	Clear documentation for user-facing functions
	Code should be clear and commented
	Module 2 methods should be implemented in package and referenced in doc
	Avoiding inefficient coding practices
	
Final Presentation (~ 25 minutes with questions)
	Similar criteria as Intro, Methods, Results, Discussion as above
	Focus is on clarity of presentation and communication 
	All students should speak, ideally for similar amounts of time
	Ability to clearly and effectively answer questions from class/myself
	Slides or markdown file for presentation is fine


# Introduction

Data does not always fall in clear-cut categories that fit pre-specified models. For example, collected data could be longitudinal and time-to-event. It would be inaccurate to only fit a longitudinal model or only fit a survival model. Previous studies (NEED CITATION) have shown that the two-step approach may reduce efficacy which in turn inflates standard errors of estimates, thus a joint model may be preferred to a two-step approach. For joint modeling, previous literature (NEED CITATION) suggests that Bayesian methods are more efficient and computationally easier. In the history of statistics, an interest in comparing frequentist versus Bayesian methods has been prominent. Thus, for our project, we aim to analyze data using a joint model approach to compare frequentist, Bayesian, and machine learning methods.
...
(1). To assess the association between CD4 cell count with time, adjusting for other covariates such as obstime, drug, prevOI (indicator of previous opportunistic infection (AIDS diagnosis) at study entry), and AZT tolerance. 
...
(3). To assess and compare the model fitting result (mean and standard error) for the frequentist and Bayesian methods. We also used Random Survival Forest for comparison. We also simulated similar data to compare the model fitting results.
...
Since this dataset contains both longitudinal and survival data, we proposed a joint model to simultaneously capture the random effects from the longitudinal data and to estimate the parameters of the survival model. We compare frequentist methods to Bayesian methods by performing analysis on aims with Monte-Carlo EM (MCEM), Bayesian methods, and finally random survival forest (RSF) to compare to machine learning methods. We are interested in seeing the accurracy and differences of these methods as well as compuational speed.
...
(1). The expected value of CD4 count is 10.2 (9.43, 10.9), for a patient at baseline, receiving ddC drug, with no previous AIDS diagnostic, and AZT tolerance.

(2). The expected change of CD4 count is -0.17 (-0.19, -0.15) when the observed time increased by one month, for the same patient.

(3). When a patient is treated with ddI, his expected CD4 count will be higher: 0.45 (-0.32, 1.20), compared with another patient treated with ddC, given the same observed time, previous AIDS diagnostic, AZT tolerance status and same random effect.

(4). When a patient has previous AIDS diagnostic, his expected CD4 count will be lower: -4.71 (-5.68, -3.74), compared with another patient without previous AIDS diagnostic, given the same observed time, same drug, AZT tolerance status and same random effect.

(5). When a patient has AZT failure, his expected CD4 count will be lower: -0.31 (-1.27, 0.60), compared with another patient with AZT tolerance, given the same observed time, same drug, same AIDS diagnostic status and same random effect.

(6). The log hazard ratio for patients with drug ddC vs. ddI is estimated to be 0.31 (0.02, 0.60), given same latent CD4 count over time, indicating ddC may increase CD4 count, by has more severe side effects than ddI.

(7). The latent CD4 count is negatively associated with hazard function: estimated log HR is -0.28 (-0.35, -0.22).
...
(3). No current method for joint modelling of longitudinal data and survival data
